...
"Model Write" 是一个面向创意写作的沉浸式 AI 编辑器。它通过结合长文本生成能力、实时上下文分析和灵 感推荐，展示模型在复杂创作场景下的深度协作能力。
...
-   **中观记忆 (Session Knowledge)**: 存储于 `entities.json`。包含 AI 提取的角色状态、地理位置特 征、关键道具列表。
...
- **核心交互**:
    ...
    - **选中改写 (Selection-triggered Rewrite)**:
        ...
        *   *交互逻辑*: 初始为禁用状态，提示“Select text to enable AI actions”。当用户选中文字时 激活，点击后针对选中区进行智能局部重写。
